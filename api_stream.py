...
class ChatGLM:
    def __init__(self, quantize_level, gpu_id) -> None:
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            "./model/chatglm2-6b-int4", trust_remote_code=True
        )
    ...
    def _model(self, quantize_level, gpu_id):
        model_name = "./model/chatglm2-6b-int4"
        ...
        if gpu_id == "-1":
            if quantize == 8:
                ...
                model_name = "./model/chatglm-6b-int4"
            elif quantize == 4:
                model_name = "./model/chatglm-6b-int4"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Stream API Service for ChatGLM2-6B-int4"
    )
    parser.add_argument(
        "--device", "-d", help="device，-1 means cpu, other means gpu ids", default="0"
    )
    parser.add_argument(
        "--quantize", "-q", help="level of quantize, option：16, 8 or 4", default=16
    )
