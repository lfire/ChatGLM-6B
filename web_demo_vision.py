...
model = (
    AutoModel.from_pretrained("THUDM/visualglm-6b", trust_remote_code=True)
    .half()
    .cuda()
)
model = model.eval()
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            top_p = gr.Slider(
                0, 1, value=0.4, step=0.01, label="Top P", interactive=True
            )
            temperature = gr.Slider(
                0, 1, value=0.8, step=0.01, label="Temperature", interactive=True
            )
    ...
    image_path.clear(
        reset_state, outputs=[image_path, chatbot, history], show_progress=True
    )
